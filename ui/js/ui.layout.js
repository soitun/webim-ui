 ...
 */
webim.layout.webapi = function(element, options){
        this._init(element, options);

};

$.extend(webim.layout.webapi.prototype, objectExtend, {
        _init: function(element, options){
                var self = this;
                var options = self.options = $.extend({}, webim.layout.webapi.defaults, options);
                if(!element){
                ...
                var ui = self.ui = {
                        ...
                        expand: element.find(".webim-expand"),
                        collapse: element.find(".webim-collapse"),
                        apps: element.find(".webim-apps"),
                        panels: element.find(".webim-panels"),
                        shortcut: element.find(".webim-shortcut")
                };
                $.extend(ui, {
                        tabsWrap: ui.panels.find(".webim-panels-tab-wrap"),
                        tabs: ui.panels.find(".webim-panels-tab"),
                        prev: ui.panels.find(".webim-panels-prev"),
                        next: ui.panels.find(".webim-panels-next")
                });
                $.extend(ui, {
                        prevMsgCount: ui.prev.find(".webim-window-tab-count"),
                        nextMsgCount: ui.next.find(".webim-window-tab-count"),
                        prevCount: ui.prev.find("span"),
                        nextCount: ui.next.find("span")
                });
                $.extend(self,{
                        window: $(window),
                        apps : {},
                        ...
                        nextCount : 0,
                        prevCount : 0,
                        menu:new webim.ui.menu(null)
 
                });
                self.addApp(self.menu, {
                        title: i18n("menu"),
                        icon: "home",
                        sticky: false,
                        onlyIcon: false,
        ...
        addApp: function(app, options, container){
                ...
                var win, el = app.element, win_el = el.parents(".webim-window:first");
                if(win_el.length){
                        win = new webim.ui.window(win_el, options);
                }
                else{
                        win = new webim.ui.window(null, options);
                        win.html(el);
                        self.ui[container ? container : "apps"].append(win.element);
                }
                app.window = win;
